...
NOTE: At the time of writing this blog, Keycloak's support for OpenID4VCI is still experimental. This guide uses the **main branch snapshot version `999.0.0-SNAPSHOT`**.
...
- **Ensure users have a password set for token acquisition**
...
[source,sh]
----
ADMIN_TOKEN=$(curl -s -X POST "https://bolankeycloak.duckdns.org/realms/master/protocol/openid-connect/token" \
...
**Crucially, we must enable the Direct Access Grant for this client**, as we intend to use the Resource Owner Password Credentials Grant (ROP-C ) flow to obtain a user access token in a subsequent step. This is done by toggling the **Direct Access Grants Enabled** switch under the *Settings* tab. All other fields are left unchanged.
...
Finally, we need to assign the previously created `membership-credential` client scope to the `oauth-sig-client` client. Since this is now possible via the Admin Console, navigate to the **Client Scopes** tab of the `oauth-sig-client` client and add `membership-credential` as an **Optional Client Scope**.

--
++++
<img src="${blogImages}/openid44vci/screenshot-client-scope-assignment.png" style="margin-bottom:2em" alt="Screenshot: Assigning membership-credential client scope to the client">
++++
--

=== Ensuring the User Has a Password

To successfully obtain a user access token using the Direct Access Grant (ROP-C) flow, the user must have a password set. If you created the user via the Admin Console without setting a password, navigate to the user's **Credentials** tab and set a password. This is a prerequisite for the next step, where we will use the user's credentials to get a token.

--
++++
<img src="${blogImages}/openid4vci/screenshot-user-set-password.png" style="margin-bottom:2em" alt="Screenshot: Setting a password for the user">
++++
--

== Obtaining a User Access Token and Credential Offer
...
This user token, which contains the `membership-credential` scope, can now be used to request a credential offer from the Keycloak issuer.

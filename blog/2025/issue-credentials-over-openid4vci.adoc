...
Because some configurations are not yet supported via the Keycloak Admin Console, we may repeatedly resort to using the Admin REST API to communicate required configurations.
...
--
++++
<img src="${blogImages}/openid4vci/screenshot-realm-and-users.png" style="margin-bottom:2em" alt="Screenshot: Created users under realm">
++++
--
...
To learn more about the **Nonce Lifetime** and other realm attributes for OpenID4VCI not accessible via the Admin Console, please refer the main https://github.com/keycloak/keycloak/blob/main/docs/documentation/server_admin/topics/oid4vci/vc-issuer-configuration.adoc[Keycloak documentation].
...
image::${blogImages}/openid4vci/screenshot-realm-ecdsa-generated-provider.png[]
...
We will configure our membership credential to be issued as an SD-JWT credential carrying a user's first name, last name, and email. Each claim to be added to the credential requires a corresponding protocol mapper from the User Model to the credential. Others fields such as the time of issuance or a unique identifier for the credential are supported via other types of protocol mappers.
...
- To learn more about other used or potential configuration attributes, please refer to the main https://github.com/keycloak/keycloak/blob/main/docs/documentation/server_admin/topics/oid4vci/vc-issuer-configuration.adoc#create-client-scopes-with-mappers[Keycloak documentation].
...
image::${blogImages}/openid4vci/screenshot-client-creation.png[]

One important point to note is that clients must explicitly enable OpenID4VCI to be able to use it.
For that, we need to set the client attribute `oid4vci.enabled` to `true`.

[source,sh]
----
curl -X PUT "http://<keycloak.instance>/admin/realms/oauth-sig-realm/clients/<client-id>" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <access-token>" \
  -d '{
    "attributes": {

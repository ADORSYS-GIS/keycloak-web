...
Others fields such as the time of issuance or a unique identifier for the credential are supported via other types of protocol mappers.
...
Lastly, we need to assign the previously created `membership-credential` client scope to the `oauth-sig-client` client as an optional client scope, so that it can issue membership credentials.

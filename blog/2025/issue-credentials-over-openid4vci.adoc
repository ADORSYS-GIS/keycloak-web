...
image::${blogImages}/openid4vci/screenshot-realm-config-oid4vci-attributes.png[]
...
We will configure our membership credential to be issued as an SD-JWT credential carrying users' name and email. Each claim to be added to the credential requires a corresponding protocol mapper from the User Model to the credential.

Because configuring a client scope for a credential type is not yet fully supported on the Admin Console, let's proceed with the Admin REST API.
...
curl -X PUT "http://<keycloak.instance>/admin/realms/oauth-sig-realm/clients/<client-id>" \
  ...
  -d '{
    ...
    "attributes": {
      ...
      "vc.issuer_did": "http://<keycloak.instance>/admin/realms/oauth-sig-realm",
      "vc.credential_build_config.sd_jwt.visible_claims": "iat",

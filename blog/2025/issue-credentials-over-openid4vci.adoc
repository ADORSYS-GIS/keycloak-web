...
Because some configurations are not yet supported via the Keycloak Admin Console, we may repeatedly resort to using the Admin REST API to convey required configurations.
...
Next, we need to define the verifiable credential that Keycloak will be able to issue to users.
...
For that, we need to set the client attribute `oid4vci.enabled` to `true`, but that cannot be done via the Admin Console at the time of writing this blog.
Therefore, we will use the Admin REST API to set this attribute for our `oauth-sig-client` client.
